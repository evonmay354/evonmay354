...
class App {
    constructor() {
        ...
        // 🎵 MUSIC START — initialize audio listener and background music
        this.listener = new THREE.AudioListener();
        this.camera.add(this.listener);

        this.backgroundMusic = new THREE.Audio(this.listener);
        this.currentMusic = "";  // Track what's currently playing
        // 🎵 MUSIC END
    ...
    // 🎵 MUSIC FUNCTION — scene-based music loader
    setSceneMusic(filename) {
        const audioLoader = new THREE.AudioLoader();
        audioLoader.load(`./assets/${filename}`, (buffer) => {
    ...
    moveDolly(dt) {
        ...
        // left, right, and ground collision
        ['x', '-x', 'y'].forEach(axis => {
            dir.set(0, 0, 0);
        ...
        if (this.renderer.xr.isPresenting) {
            ...
            if (this.selectPressed || moveGaze) {
                ...
                if (this.boardData) {
                    ...
                    // 🎵 MUSIC ZONE TRIGGERS
                    const lobbyObj = this.scene.getObjectByName("LobbyShop");
                    const lobbyPos = lobbyObj ? lobbyObj.getWorldPosition(new THREE.Vector3()) : null;

                    if (lobbyPos && dollyPos.distanceTo(lobbyPos) < 5 && this.currentMusic !== "lobby.mp3") {
                        this.currentMusic = "lobby.mp3";
                        this.setSceneMusic(this.currentMusic);
                    } else if (dollyPos.z < -5 && this.currentMusic !== "hallway.mp3") {
                        this.currentMusic = "hallway.mp3";
                        this.setSceneMusic(this.currentMusic);
                    } else if ((!lobbyPos || dollyPos.distanceTo(lobbyPos) >= 5) && dollyPos.z >= -5 && this.currentMusic !== "default.mp3") {
                        this.currentMusic = "default.mp3";
                        this.setSceneMusic(this.currentMusic);
                    }
